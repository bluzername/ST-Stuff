...
function createSvgObjectsFromPcbTrace(trace, ctx) {
  ...
  for (const [start, end] of segments) {
    const startPoint = applyToPoint25(transform, [start.x, start.y]);
    const endPoint = applyToPoint25(transform, [end.x, end.y]);
...
function createAnchorOffsetIndicators(params) {
  ...
  const objects = [];
  const [screenGroupAnchorX, screenGroupAnchorY] = applyToPoint27(transform, [
    groupAnchorPosition.x,
    groupAnchorPosition.y
  ]);
  const [screenComponentX, screenComponentY] = applyToPoint27(transform, [
    componentPosition.x,
    componentPosition.y
  ]);
...
function createSvgObjectsForRatsNest(circuitJson, ctx) {
  ...
  for (const line of ratsNestLines) {
    const transformedStart = applyToPoint32(transform, [
      line.startPoint.x,
      line.startPoint.y
    ]);
    const transformedEnd = applyToPoint32(transform, [
      line.endPoint.x,
      line.endPoint.y
    ]);
    const attributes = {
...
function createSvgObjectsFromPcbKeepout(keepout, ctx) {
  ...
  for (const layer of keepout.layers) {
    ...
    if (keepout.shape === "rect") {
      ...
      const [cx, cy] = applyToPoint35(transform, [
...
function ringToPathD(vertices, transform) {
  ...
  const transformedVertices = vertices.map((v) => {
    const [x, y] = applyToPoint36(transform, [v.x, v.y]);
    return { ...v, x, y };
  });
...
function createSvgObjectsFromPcbCopperPour(pour, ctx) {
  ...
  if (pour.shape === "rect") {
    const [cx, cy] = applyToPoint37(transform, [pour.center.x, pour.center.y]);
  ...
  if (pour.shape === "polygon") {
    ...
    const transformedPoints = pour.points.map(
      (p) => applyToPoint37(transform, [p.x, p.y])
    );
    const pointsString = transformedPoints.map((p) => `${p[0]},${p[1]}`).join(" ");
    const copperPolygon = {
      name: "polygon",
...
function createSvgObjectsFromPcbGroup(pcbGroup, ctx) {
  ...
  const halfWidth = width / 2;
  const halfHeight = height / 2;
  const [topLeftX, topLeftY] = applyToPoint39(transform, [
    center.x - halfWidth,
    center.y + halfHeight
  ]);
  const [bottomRightX, bottomRightY] = applyToPoint39(transform, [
    center.x + halfWidth,
    center.y - halfHeight
  ]);
  const rectX = Math.min(topLeftX, bottomRightX);
...
function createSvgObjectsFromAssemblyComponent(params, ctx) {
  ...
  const [pinX, pinY] = applyToPoint43(transform, [portPosition.x, portPosition.y]);
  const scaledWidth = width * Math.abs(transform.a);
  const scaledHeight = height * Math.abs(transform.d);
...
function createSvgObjectsFromAssemblyPlatedHole(hole, ctx) {
  ...
  if (hole.shape === "circular_hole_with_rect_pad") {
    ...
    const holeRadius = scaledHoleDiameter / 2;
    const [holeCx, holeCy] = applyToPoint45(transform, [
  ...
  if (hole.shape === "pill_hole_with_rect_pad") {
    ...
    const holeOffsetX = pillHoleWithOffsets.hole_offset_x ?? 0;
    const holeOffsetY = pillHoleWithOffsets.hole_offset_y ?? 0;
    const [holeCenterX, holeCenterY] = applyToPoint45(transform, [
      pillHole.x + holeOffsetX,
      pillHole.y + holeOffsetY
    ]);
    const holeRadius = Math.min(scaledHoleHeight, scaledHoleWidth) / 2;
...
function createSvgObjectsFromPinoutBoard(pcbBoard, ctx) {
  ...
  if (outline && Array.isArray(outline) && outline.length >= 3) {
    path = outline.map((point, index) => {
      const [x, y] = applyToPoint48(transform, [point.x, point.y]);
      return index === 0 ? `M ${x} ${y}` : `L ${x} ${y}`;
    }).join(" ");
  } else {
    const halfWidth = width / 2;
    const halfHeight = height / 2;
    const topLeft = applyToPoint48(transform, [
      center.x - halfWidth,
      center.y - halfHeight
    ]);
    const topRight = applyToPoint48(transform, [
      center.x + halfWidth,
      center.y - halfHeight
    ]);
    const bottomRight = applyToPoint48(transform, [
      center.x + halfWidth,
      center.y + halfHeight
    ]);
    const bottomLeft = applyToPoint48(transform, [
      center.x - halfWidth,
      center.y + halfHeight
    ]);
    path = `M ${topLeft[0]} ${topLeft[1]} L ${topRight[0]} ${topRight[1]} L ${bottomRight[0]} ${bottomRight[1]} L ${bottomLeft[0]} ${bottomLeft[1]}`;
  ...
  for (const cutout of cutlery) {
    if (cutout.shape === "rect") {
      ...
      const [tl, tr, br, bl] = [
        applyToPoint48(transform, [x - halfWidth, y - halfHeight]),
        applyToPoint48(transform, [x + halfWidth, y - halfHeight]),
        applyToPoint48(transform, [x + halfWidth, y + halfHeight]),
        applyToPoint48(transform, [x - halfWidth, y + halfHeight])
      ];
...
function createSvgObjectsFromPinoutComponent(elm, ctx) {
  ...
  const [x, y] = applyToPoint49(transform, [center.x, center.y]);
  const scaledWidth = width * Math.abs(transform.a);
  const scaledHeight = height * Math.abs(transform.d);
...
function createSvgObjectsFromPinoutHole(hole, ctx) {
  const { transform } = ctx;
  const [x, y] = applyToPoint50(transform, [hole.x, hole.y]);
  if (hole.hole_shape === "circle" || hole.hole_shape === "square") {
    const scaledDiameter = hole.hole_diameter * Math.abs(transform.a);
    const radius = scaledDiameter / 2;
    if (hole.hole_shape === "circle") {
...
function createSvgObjectsFromPinoutPlatedHole(hole, ctx) {
  const { transform } = ctx;
  const [x, y] = applyToPoint51(transform, [hole.x, hole.y]);
  if (hole.shape === "pill") {
    const scaledOuterWidth = hole.outer_width * Math.abs(transform.a);
    const scaledOuterHeight = hole.outer_height * Math.abs(transform.a);
    const scaledHoleWidth = hole.hole_width * Math.abs(transform.a);
...
function createSvgObjectsFromPinoutSmtPad(pad, ctx) {
  ...
  if (pad.shape === "rect" || pad.shape === "rotated_rect") {
    const width = pad.width * Math.abs(transform.a);
    const height = pad.height * Math.abs(transform.d);
    const [x, y] = applyToPoint52(transform, [pad.x, pad.y]);
    if (pad.shape === "rotated_rect" && pad.ccw_rotation) {
      return [
        {
          name: "rect",
  ...
  if (pad.shape === "polygon") {
    const points = (pad.points ?? []).map(
      (point) => applyToPoint52(transform, [point.x, point.y])
    );
    return [
      {
        name: "polygon",
...
function createSvgObjectsFromPinoutPort(pcb_port, ctx) {
  ...
  const [port_x, port_y] = applyToPoint53(ctx.transform, [pcb_port.x, pcb_port.y]);
...
function calculateVerticalEdgeLabels(edge, pinout_labels, {
  ...
  const board_edge_x = applyToPoint54(transform, [
...
function drawSchematicGrid(params) {
  ...
  const transformPoint = (x, y) => {
    const [transformedX, transformedY] = applyToPoint55(params.transform, [x, y]);
...
function drawSchematicLabeledPoints(params) {
  ...
  for (const point of points) {
    const [x1, y1] = applyToPoint56(transform, [point.x - 0.1, point.y - 0.1]);
    const [x2, y2] = applyToPoint56(transform, [point.x + 0.1, point.y + 0.1]);
    const [x3, y3] = applyToPoint56(transform, [point.x - 0.1, point.y + 0.1]);
    const [x4, y4] = applyToPoint56(transform, [point.x + 0.1, point.y - 0.1]);
    ...
    const [labelX, labelY] = applyToPoint56(transform, [
      point.x + 0.15,
      point.y - 0.15
    ]);
    labeledPointsGroup.push({
...
var createSvgSchErrorText = ({
  ...
  const screenCenter = applyToPoint57(realToScreenTransform, realCenter);
  return {
    type: "element",
    name: "text",
    value: "",
...
var createSvgObjectsFromSchematicComponentWithSymbol = ({
  ...
  for (const port of symbol.ports) {
    ...
    const screenPortPos = applyToPoint58(
      compose11(realToScreenTransform, transformFromSymbolToReal),
      port
    );
    svgObjects.push({
...
var createSvgObjectsForSchPortBoxLine = ({
  ...
  const screenRealEdgePos = applyToPoint59(transform, realEdgePos);
  ...
  const screenLineEnd = applyToPoint59(transform, realLineEnd);
...
var createSvgObjectsForSchPortPinNumberText = (params) => {
  ...
  const screenPinNumberTextPos = applyToPoint60(transform, realPinNumberPos);
  svgObjects.push({
    name: "text",
    type: "element",
    attributes: {
...
var createSvgObjectsForSchPortPinLabel = (params) => {
  ...
  const screenPinNumberTextPos = applyToPoint61(transform, realPinNumberPos);
...
var createSvgSchText = ({
  ...
  const center = applyToPoint63(transform, elm.position);
  const textAnchorMap = {
    center: "middle",
    center_right: "end",
    bottom_left: "start",
...
var createSvgObjectsFromSchematicComponentWithBox = ({
  ...
  const svgObjects = [];
  const componentScreenTopLeft = applyToPoint64(transform, {
    x: schComponent.center.x - schComponent.size.width / 2,
    y: schComponent.center.y + schComponent.size.height / 2
  });
  const componentScreenBottomRight = applyToPoint64(transform, {
    x: schComponent.center.x + schComponent.size.width / 2,
    y: schComponent.center.y - schComponent.size.height / 2
  });
...
function createSvgObjectsFromSchDebugObject({
  ...
  if (debugObject.shape === "rect") {
    let [screenLeft, screenTop] = applyToPoint66(transform, [
      debugObject.center.x - debugObject.size.width / 2,
      debugObject.center.y - debugObject.size.height / 2
    ]);
    let [screenRight, screenBottom] = applyToPoint66(transform, [
      debugObject.center.x + debugObject.size.width / 2,
      debugObject.center.y + debugObject.size.height / 2
    ]);
    ...
    const [screenCenterX, screenCenterY] = applyToPoint66(transform, [
  ...
  if (debugObject.shape === "line") {
    const [screenStartX, screenStartY] = applyToPoint66(transform, [
      debugObject.start.x,
      debugObject.start.y
    ]);
    const [screenEndX, screenEndY] = applyToPoint66(transform, [
      debugObject.end.x,
      debugObject.end.y
    ]);
...
function createSchematicTrace({
  ...
  for (let edgeIndex = 0; edgeIndex < edges.length; edgeIndex++) {
    ...
    const [screenFromX, screenFromY] = applyToPoint67(transform, [
      edge.from.x,
      edge.from.y
    ]);
    const [screenToX, screenToY] = applyToPoint67(transform, [
      edge.to.x,
      edge.to.y
    ]);
  ...
  if (trace.junctions) {
    for (const junction of trace.junctions) {
      const [screenX, screenY] = applyToPoint67(transform, [
        junction.x,
        junction.y
      ]);
      overlayObjects.push({
...
var createSvgObjectsForSchNetLabelWithSymbol = ({
  ...
  const rotatedSymbolEnd = applyToPoint68(rotationMatrix, symbolEndPoint);
  ...
  const [screenMinX, screenMinY] = applyToPoint68(
    compose12(realToScreenTransform, symbolToRealTransform),
    [bounds.minX, bounds.minY]
  );
  const [screenMaxX, screenMaxY] = applyToPoint68(
    compose12(realToScreenTransform, symbolToRealTransform),
    [bounds.maxX, bounds.maxY]
  );
  const rectHeight = Math.abs(screenMaxY - screenMinY);
  ...
  for (const text of symbolTexts) {
    const screenTextPos = applyToPoint68(
      compose12(realToScreenTransform, symbolToRealTransform),
      text
    );
  ...
  for (const box of symbolBoxes) {
    const screenBoxPos = applyToPoint68(
      compose12(realToScreenTransform, symbolToRealTransform),
      box
    );
  ...
  for (const circle of symbolCircles) {
    const screenCirclePos = applyToPoint68(
      compose12(realToScreenTransform, symbolToRealTransform),
      circle
    );
    const symbolToScreenScale = compose12(
...
var createSvgObjectsForSchNetLabel = ({
  ...
  const fontSizeMm = getSchMmFontSize("net_label");
  const textWidthFSR = estimateTextWidth(labelText || "");
  const screenCenter = applyToPoint69(realToScreenTransform, schNetLabel.center);
  const realTextGrowthVec = getUnitVectorFromOutsideToEdge(
    schNetLabel.anchor_side
  );
  ...
  const fullWidthFsr = textWidthFSR + ARROW_POINT_WIDTH_FSR * 2 + END_PADDING_EXTRA_PER_CHARACTER_FSR * labelText.length + END_PADDING_FSR;
  const screenAnchorPosition = schNetLabel.anchor_position ? applyToPoint69(realToScreenTransform, schNetLabel.anchor_position) : {
  ...
  const screenOutlinePoints = [
    ...
    (fontRelativePoint) => applyToPoint69(
      compose13(
        realToScreenTransform,
...
var createSvgObjectsFromSchematicBox = ({
  ...
  const topLeft = applyToPoint70(transform, {
    x: schematicBox.x,
    y: schematicBox.y
  });
  const bottomRight = applyToPoint70(transform, {
...
var createSvgObjectsFromSchematicTable = ({
  ...
  const [screenTopLeftX, screenTopLeftY] = applyToPoint71(transform, [
    topLeftX,
    topLeftY
  ]);
  const [screenBottomRightX, screenBottomRightY] = applyToPoint71(transform, [
    topLeftX + totalWidth,
    topLeftY - totalHeight
  ]);
  svgObjects.push({
  ...
  for (let i = 0; i < column_widths.length - 1; i++) {
    ...
    for (let j = 0; j < row_heights.length; j++) {
      ...
      if (!isMerged) {
        const start = applyToPoint71(transform, { x: currentX, y: segmentStartY });
        const end = applyToPoint71(transform, { x: currentX, y: segmentEndY });
        svgObjects.push({
          name: "line",
          type: "element",
          attributes: {
  ...
  for (let i = 0; i < row_heights.length - 1; i++) {
    ...
    for (let j = 0; j < column_widths.length; j++) {
      ...
      if (!isMerged) {
        const start = applyToPoint71(transform, {
          x: segmentStartX,
          y: currentY
        });
        const end = applyToPoint71(transform, { x: segmentEndX, y: currentY });
        svgObjects.push({
          name: "line",
          type: "element",
          attributes: {
  ...
  for (const cell of cells) {
    if (cell.text) {
      ...
      const screenTextAnchorPos = applyToPoint71(transform, realTextAnchorPos);
...
function createSvgObjectsFromSchematicLine({
  ...
  const p1 = applyToPoint73(transform, { x: schLine.x1, y: schLine.y1 });
  const p2 = applyToPoint73(transform, { x: schLine.x2, y: schLine.y2 });
...
function createSvgObjectsFromSchematicCircle({
  ...
  const center = applyToPoint74(transform, schCircle.center);
...
function createSvgObjectsFromSchematicRect({
  ...
  const center = applyToPoint75(transform, schRect.center);
...
function createSvgObjectsFromSchematicArc({
  ...
  const center = applyToPoint76(transform, schArc.center);
...
function createSvgObjectsFromSchematicPath({
  ...
  const transformedPoints = schPath.points.map(
    (p) => applyToPoint77(transform, { x: p.x, y: p.y })
  );
...
import {
  applyToPoint as applyToPoint80,
...
function createSvgObjectsFromSolderPaste(solderPaste, ctx) {
  ...
  const [x, y] = applyToPoint79(transform, [solderPaste.x, solderPaste.y]);
...
function createSvgObjectFromPcbBoundary2(transform, minX, minY, maxX, maxY) {
  const [x1, y1] = applyToPoint80(transform, [minX, minY]);
  const [x2, y2] = applyToPoint80(transform, [maxX, maxY]);
  const width = Math.abs(x2 - x1);
  const height = Math.abs(y2 - y1);
  const x = Math.min(x1, x2);
  const y = Math.min(y1, y2);
