...
var getApertureConfigFromPcbVia = (elm) => {
...
var convertSoupToGerberCommands = (soup, opts = {}) => {
  ...
  for (const element of soup) {
    if (element.type === "pcb_plated_hole" && "x" in element && typeof element.x === "number" && "y" in element && typeof element.y === "number") {
      const rotation = "ccw_rotation" in element && typeof element.ccw_rotation === "number" ? element.ccw_rotation : 0;
    ...
    for (const element of soup) {
      if (element.type === "pcb_trace") {
        ...
        if (element.layer === layer) {
          ...
          const rotation = "ccw_rotation" in element && typeof element.ccw_rotation === "number" ? element.ccw_rotation : rotationLookup.get(`${element.x}:${element.y}`) ?? 0;
          const gb = gerberBuilder().add("select_aperture", {
            aperture_number
          });
          if (rotation) {
        ...
        if (element.layers.includes(layer)) {
          for (const glayer of [
            ...
            if (element.shape === "pill") {
              ...
              const rotation = "rect_ccw_rotation" in element && typeof element.rect_ccw_rotation === "number" ? element.rect_ccw_rotation : void 0;
              const gb = gerberBuilder().add("select_aperture", {
                aperture_number: findApertureNumber(glayer, apertureConfig)
              });
              if (rotation)
...
//# sourceMappingURL=chunk-WSRKOLQZ.js.map